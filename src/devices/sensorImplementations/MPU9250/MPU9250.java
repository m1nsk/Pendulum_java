...
public class MPU9250 extends NineDOF
{
    ...
    private void calibrateGyroAcc() throws IOException, InterruptedException
    {
        ...
        mpu9250.write(Registers.I2C_MST_CTRL.getAddress(),(byte) 0x00); // Disable Protocol master
        mpu9250.write(Registers.USER_CTRL.getAddress(),(byte) 0x00);    // Disable FIFO and Protocol master modes
    ...
    private void initMPU9250() throws IOException, InterruptedException
    {
        ...
        mpu9250.write(Registers.SMPLRT_DIV.getAddress(), (byte)0x03);  // Use a 200 Hz rate; a rate consistent with the filter update rate
...
// c = c & ~0xE0; // Clear self-test bits [7:5]
        ...
        // can join the Protocol bus and all can be controlled by the Arduino as master
...
//        System.out.println("Accelerometer " + registers[0] + ", " + registers[1] + ", " + registers[2]);
...
//        registers = read16BitRegisters(mpu9250,Registers.GYRO_XOUT_H.getAddress(),3);
//        System.out.println("Gyroscope " + registers[0] + ", " + registers[1] + ", " + registers[2]);

        x = (float) ((float)registers[3]*gyrScale.getRes()); // transform from raw data to degrees/s
        y = (float) ((float)registers[4]*gyrScale.getRes()); // transform from raw data to degrees/s
        z = (float) ((float)registers[5]*gyrScale.getRes()); // transform from raw data to degrees/s
